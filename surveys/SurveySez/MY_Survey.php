	...
	public static function responseList($id)
	{
        ...
        <table class="table table-hover">
          ...
          ';
            ...
            while($row = mysqli_fetch_assoc($result))#while loop repeater
            {# process each row
                 //$myReturn .= '<div align="center"><a href="' . VIRTUAL_PATH . 'surveys/survey_view.php?id=' . (int)$row['SurveyID'] . '">' . dbOut($row['Title']) . '</a></div>';
            $myReturn .= '
            <tr class="table-info">
              <td><a href="' . VIRTUAL_PATH . 'surveys/response_view.php?id=' . (int)$row['ResponseID'] . '"><button type="button" class="btn btn-primary">' . dbOut($row['DateAdded']) . '</button></a></td>
            </tr>
            ';
